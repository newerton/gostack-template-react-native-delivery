...
const FoodDetails: React.FC = () => {
  ...
  useEffect(() => {
    async function loadFood(): Promise<void> {
      ...
      const favorite = data.find(
        (findFavorite: Params) => findFavorite.id === routeParams.id,
      );
  ...
  function handleIncrementExtra(id: number): void {
    ...
    const newExtras = extras.filter(removableExtra => removableExtra.id !== id);
  ...
  const cartTotal = useMemo(() => {
    // Calculate cartTotal
    const totalExtras = extras.reduce((cartSum: number, extra: Extra) => {
      cartSum += extra.quantity * extra.value;
      return cartSum;
    }, 0);
